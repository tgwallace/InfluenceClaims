...
public class PlotWhitelistCity implements CommandExecutor {
    private final InfluenceClaims plugin;
    public PlotWhitelistCity(InfluenceClaims plugin){
        this.plugin = plugin;
    }
    @Override
        ...
        for(String name : strings) {
            ...
            if(!playerData.contains(recipient.getUniqueId().toString())){
                ...
                if(messageRecipient != null) {
                    messageRecipient.sendRawMessage(sender.getName() + " has added you to a plot located at " + plotData[1] + " in the city of " + cityData.getString(plotData[0]+".Name") + "!");
                } else {
                    plugin.playerMessage(recipient.getUniqueId().toString(),sender.getName() + " has added you to a plot located at " + plotData[1] + " in the city of " + cityData.getString(plotData[0]+".Name") + "!");
                }
                sender.sendRawMessage(name + " has been added to the whitelist for this plot!");
            }
        }
        plugin.saveCityData();
        return true;
