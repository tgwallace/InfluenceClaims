...
public class PlotRemoveCity implements CommandExecutor {
    private final InfluenceClaims plugin;
    public PlotRemoveCity(InfluenceClaims plugin){
        this.plugin = plugin;
    }
    @Override
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        for(String name : strings) {
            ...
            if(!whitelist.contains(recipient.getUniqueId().toString())){
                sender.sendRawMessage("Could not locate a player named " + name + " on the whitelist for this plot.");
                return true;
            }

            whitelist.remove(recipient.getUniqueId().toString());
            ...
            if(messageRecipient != null) {
                messageRecipient.sendRawMessage(sender.getName() + " has removed you from a plot located at " + plotData[1] + " in the city of " + cityData.getString(plotData[0]+".Name") + "!");
            } else {
                plugin.playerMessage(recipient.getUniqueId().toString(),sender.getName() + " has removed you from a plot located at " + plotData[1] + " in the city of " + cityData.getString(plotData[0]+".Name") + "!");
            }
            sender.sendRawMessage(name + " has been removed from the whitelist for this plot!");
        }
        plugin.saveCityData();
        return true;
    }
