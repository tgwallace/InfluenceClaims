...
public class CityElectionManager {
    ...
    public List<String> getElectorate(String cityUUID, String issue) {
        ...
        electorate.removeIf(playerUUID -> ChronoUnit.DAYS.between(LocalDate.parse(playerData.getString(playerUUID + ".LastLogin")), LocalDate.now()) >= plugin.getConfig().getInt("AbsenteeTimer"));
        return electorate;
    }
}
